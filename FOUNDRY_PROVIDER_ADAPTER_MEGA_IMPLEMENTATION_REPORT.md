...
- **PrimeOpp** (`C:\Users\jp718\Documents\GitHub\PrimeOpp`): the DNS advisory fixture reflects `PRIMEOPP_DOMAIN_READINESS.md`'s actual finding that no domain is purchased and, per its `.replit` config, Replit's own autoscale is the app's **current dev-stack host only** — **not** a Railway/Fly/Vercel deployment, and *not* a deployment recommendation of any kind (see `FOUNDRY_REPLIT_CLASSIFICATION_CURRENT_TRUTH.md`; the fixture's `replitClassification` field now records this explicitly: `status: dev_stack_origin_only`, `deploymentTargetStatus: undecided`). This ruled out an earlier draft plan to frame PrimeOpp's env-update fixtures around Railway/Fly/Vercel as if that were PrimeOpp's actual infrastructure; those fixtures were kept explicitly synthetic (`project: "foundry-ops-demo"`) instead.
